...
function rain.new(group, config)
	...
	if (config.snow) then
		...
		rain.dropWidth = config.width or 1    -- witdh
		rain.dropAlpha = config.alpha or 0.08 -- alpha value
		rain.dropColor = config.color or 255 -- color
	end
